...
`pro.haichuang.framework`是一个简易 [spring-boot](https://github.com/spring-projects/spring-boot) 工具封装。它使用了较新的依赖版本，在 [spring-boot](https://github.com/spring-projects/spring-boot) 的基础上进一步封装了常见的基本配置，开箱即用，统一了开发风格，减少了大部分重复开发的工作，同时也让维护更加容易，如果在使用中遇到了任何问题可以提交 `Issues` ，我们将尽快优化，希望本项目能帮助到您。
...
1. `spring.servlet.multipart.max-file-size` ，默认值=`100GB`
2. `spring.servlet.multipart.max-request-size` ，默认值=`100GB`
...
- `@EnableLogSave`：启动请求`AOP`日志持久化，需要在接口上标注`pro.haichuang.framework.base.config.annotation.LogSave`注解，再自定义继承`pro.haichuang.framework.base.config.interceptor.AbstractLogSave`抽象类实现逻辑
...
- 分页查询基类：`pro.haichuang.framework.base.query.Page<p align="center">
  <img width="320" src="https://www.haichuang.pro/upload/202010/1602729036.png">
</p>
...
**项目依赖版本禁止修改|项目依赖版本禁止修改|项目依赖版本禁止修改**
...
```
|pro.haichuang.framework.service.项目代号----------------主包名	
|  config-------------------------------------------------配置包
|    properties---------------------------------------------配置文件（映射yml文件中自定义配置）
...
> 所有实体类必须继承此类，详细请参考：`pro.haichuang.framework.mybatis.domain.BaseDO`
